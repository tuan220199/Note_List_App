...
function Modal({ mode, setShowModal, getData, task }) {
  ...
  const postData = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`https://app-list-backend-hcchdyh2agg5hhgx.westeurope-01.azurewebsites.net/todos`, {
        method: "POST",
  ...
  const editData = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`https://app-list-backend-hcchdyh2agg5hhgx.westeurope-01.azurewebsites.net/todos/${task.id}`, {
        method: "PUT",
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      });
